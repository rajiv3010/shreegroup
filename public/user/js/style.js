...
$( ".TreeGetDetails" ).mouseover(function() {
    ...
    $("#user_name").keyup(function (e) {
        ...
        if (user_name.length > 0) {
            $(".user_name_search").html("<span class='badge badge-info'>Searching</span>");
            var token = $('meta[name="_token"]').attr("content");
            e.preventDefault();
            jQuery.ajax({
                ...
                success: function (result) {
                    if (result==0) {
                        $(".user_name_search").html("<span class='badge badge-success'>User name availabel</span>");
                    } else {
                        $(".user_name_search").html("<span class='badge badge-danger'>User name already exist</span>");
                    }
                },
            });
        } else {
    ...
    $("#discount").keyup(function() {
     ...
     $("#parent_key").keyup(function() {
        $("#parent_name").html('<p>Searching...</p>');
        var e = $(this).val();
        if (e.length >= 6){
          ...
          $.ajax({
            ...
            if (a.name==undefined){
              $("#parent_name").html('<p>Invalid sponsor key</p>');
            }else{

              $("#parent_name").html('<p>'+a.name+'</p>');
            }
     ...
     $("#sponsor_key").keyup(function() {
        $("#sponser_name").html('<p>Searching...</p>');
        var e = $(this).val();
        if (e.length >= 6){
          ...
          $.ajax({
            ...
            var a = jQuery.parseJSON(e);
            if (a.name==undefined){
              $("#sponser_name").html('<p>Invalid sponsor key</p>');
            }else{

              $("#sponser_name").html('<p>'+a.name+'</p>');
            }
...
/***********************************************/
 ...
 $("#productBrand").change(function() {
    ...
    $.ajax({
    type: "get",
    url: "/getState?cid="+val,
    beforeSend: function() {
    $(".loader").html("<p>Please wait...</p>");
    },
